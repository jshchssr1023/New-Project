...
export const shopRulesApi = {
  ...
  getAll: async (): Promise<Array<{
    ...
    const response = await apiClient.get('/shop-rules');
    return response.data;
  },

  // Get rule type schemas
  getSchemas: async (): Promise<Record<string, {
    label: string;
    description: string;
    conditions: { key: string; type: string; label: string; default: unknown }[];
    actions: { key: string; type: string; label: string; default: unknown }[];
  }>> => {
    const response = await apiClient.get('/shop-rules/schemas');
    return response.data;
  },

  // Get default rules template
  getDefaults: async (): Promise<Array<{
    id: string;
    name: string;
    description: string;
    ruleType: string;
    ...
    const response = await apiClient.get('/shop-rules/defaults');
    return response.data;
  },
  ...
  create: async (rule: {
    ...
    const response = await apiClient.post('/shop-rules', rule);
    return response.data;
  },

  // Update a rule
  update: async (id: string, rule: Partial<{
    name: string;
    description: string;
    ruleType: string;
    priority: number;
    ...
    const response = await apiClient.put(`/shop-rules/${id}`, rule);
    return response.data;
  },
  ...
  toggle: async (id: string): Promise<{
    id: string;
    name: string;
    description: string;
    ruleType: string;
    ...
    const response = await apiClient.post(`/shop-rules/${id}/toggle`);
    return response.data;
  },
  ...
  test: async (carId: string, month: string): Promise<{
    carId: string;
    carNumber: string;
    ...
    const response = await apiClient.post('/shop-rules/test', { carId, month });
    return response.data;
  },

  // Reset to default rules
  resetToDefaults: async (): Promise<Array<{
    id: string;
    name: string;
    description: string;
    ruleType: string;
    ...
    const response = await apiClient.post('/shop-rules/reset-defaults');
    return response.data;
  },
};
